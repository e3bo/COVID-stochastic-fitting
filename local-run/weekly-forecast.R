...
params <- readRDS("output/2020-11-17/parameter-estimates-Georgia.rds")
...
pn1 <- readRDS("./output/current/Georgia-params-natural.rds") %>% slice(1:3)
pn2 <- readRDS("./output/current/Georgia-params-natural.rds") %>%
  filter(is_fitted == "yes")

assumed_fitted <- c("min_frac_dead", "max_frac_dead", "log_half_dead", "theta_cases", 
  "theta_deaths", "sigma_dw", "b1", "b2", "b3", "b4", "b5", "b6", 
  "b7", "b8", "b9", "b10", "b11", "b12", "E1_0", "Ia1_0", "Isu1_0", 
  "Isd1_0")
stopifnot(isTRUE(setequal(rownames(pn2), assumed_fitted)))

pn <- bind_rows(pn1, pn2) %>% select(-is_fitted) %>% t() %>%
  as_tibble() %>%
  mutate(log_theta_cases = log(theta_cases),
...
#pomp_res$all_partable <- as_tibble(as.list(c(LogLik = 0, MIF_ID = NA, LogLik_SE = NA, params)))
