...
scef <- evaluate_predictions(scef[1], backfill_buffer = 0)
erwf <- evaluate_predictions(srwf[1], backfill_buffer = 0)


ei <- evalcast:::intersect_locations(c(erwf, esqs, elis, erns, scef))
plot_measure(ei, "ae")
plot_measure(ei, "wis")
plot_width(ei)
...
scef2 <- evaluate_predictions(scef[2], backfill_buffer = 0)

ei2 <- evalcast:::intersect_locations(c(erwf2, esqs2, elis2, erns2, eutf, scef2))
plot_measure(ei2, "ae")
