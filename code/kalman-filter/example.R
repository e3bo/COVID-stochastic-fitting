...
reactions <- list(infectS=list(paste0("rate = psi * phi * 1 / Nthis * beta * ((Isd1 + Isd2 + Isd3 + Isd4)",
                                     ...
                                     " + bIa * (Ia1 + Ia2 + Ia3 + Ia4)",
                                     " + bC * (C1 + C2 + C3 + C4)",
                                     " + bH * (H1 + H2 + H3 + H4));"), c(S=-1, L1=1)),
                 exitL1=list("rate = L1 * gammaL;", c(L1=-1, L2 = 1)),
                 exitL2=list("rate = L2 * gammaL;", c(L2=-1, L3 = 1)),
                 exitL3=list("rate = L3 * gammaL;", c(L3=-1, L4 = 1)),
                 enterIa1=list("rate = L4 * gammaL * a;", c(L4=-1, Ia1 = 1)),
                 enterIsu1=list("rate = L4 * gammaL * (1 - a) * (1 - q);", c(L4=-1, Isu1 = 1)),
                 enterIsd1=list("rate = L4 * gammaL * (1 - a) * q;", c(L4=-1, Isd1 = 1)),
